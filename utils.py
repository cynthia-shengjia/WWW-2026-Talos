...
def write_result(res_path, res_stat, model_condig):
    file_header = None
    file_stats = None
    f = None
    if model_condig['loss'] == 'renyi':
        if model_condig['mode'] == 'CCL':
            file_header = '|   neg_weight | layer | margin | recall@20  |    NDCG   | N-neg    |  lr | weight_decay | model  | train_batch |\n'
            file_stats = '|    {}   |    {}    |  {}    |   {}  |  {}   |  {}   |  {}   |  {}  |  {}  | {}  | {}   |   {}   |\n'.format(
                format(model_condig['neg_coefficient'],'.3f'),model_condig['n_layers'],
                format(model_condig['renyi_margin'],'.3f'), format(res_stat['recall'],'.6f'), format(res_stat['ndcg'],'.6f'), model_condig['num_negtive_items'],
                format(model_condig['lr'],'.1'),format(model_condig['weight_decay'],'.1'),
                model_condig['model'], model_condig['train_batch'], format(res_stat['test_recall'],'.6f'), format(res_stat['test_ndcg'],'.6f')
            )
        else:
            file_header = '|   omega   |    neg_weight    |    layer      | margin   |    recall@20     |    NDCG   | N-neg    |   mode     |  optimizer  |\n'
            file_stats = '|   {}   |    {}   |    {}    |   {}     |  {}    |      {}     |    {}   |    {}   |  {}   |  {}   |  {}   |  {}   | {}   | {}   |   {}   |\n'.format(
                format(model_condig['renyi_omega'],'.3f'), format(model_condig['neg_coefficient'],'.3f'),model_condig['n_layers'],
                format(model_condig['renyi_margin'],'.3f'), format(res_stat['recall'],'.6f'), format(res_stat['ndcg'],'.6f'), model_condig['num_negtive_items'],
                model_condig['mode'],model_condig['search_optimizer'],format(model_condig['lr'],'.1'),format(model_condig['lr2'],'.1'),
                model_condig['model'],format(model_condig['weight_decay'],'.1'), format(res_stat['test_recall'],'.6f'), format(res_stat['test_ndcg'],'.6f')
            )
    elif model_condig['loss'] == 'llpauc':
        file_header = '|   alpha   |     beta   |   layer   |      N-neg    |      recall@20     |    NDCG   |   lr  |  weight_decay | model  | optimizer| trainbatch |\n'
        file_stats = '|   {}  |    {}   |    {}   |   {}   |   {}    |  {}  |  {}   |  {}   |{}   |  {}   | {}   | {}   |   {}   |\n'.format(
            format(model_condig['alpha'],'.3f'),format(model_condig['beta'],'.3f'),  model_condig['n_layers'],model_condig['num_negtive_items'],
            format(res_stat['recall'],'.6f'), format(res_stat['ndcg'],'.6f'),
            format(model_condig['lr'],'.1'),format(model_condig['weight_decay'],'.1'),
            model_condig['model'], model_condig['search_optimizer'], model_condig['train_batch'], format(res_stat['test_recall'],'.6f'), format(res_stat['test_ndcg'],'.6f')
        )
    elif model_condig['loss'] == 'advinfonce':
        file_header = '|   tmp   |   layer      | k_neg    |      N-neg    |      recall@20     |    NDCG   |   lr | adv_lr | model |  w_emb_dim | eta_epochs | adv_interval | train_batch |\n'
        file_stats = '|   {}   |    {}   |    {}    |    {}   |   {}    |  {}  |  {}   | {}   |   {}   | {}   | {}   |   {}   |  {}   | {}   |   {}   |\n'.format(
            format(model_condig['ssm_temp'],'.2f'), model_condig['n_layers'], format(model_condig['neg_coefficient'],'.3f'),  model_condig['num_negtive_items'],
            format(res_stat['recall'],'.6f'), format(res_stat['ndcg'],'.6f'),
            format(model_condig['lr'],'.1'),format(model_condig['adv_lr'],'.1'),
            model_condig['model'], model_condig['w_emb_dim'], model_condig['eta_epochs'],
            model_condig['adv_interval'], model_condig['train_batch'], format(res_stat['test_recall'],'.6f'), format(res_stat['test_ndcg'],'.6f')
        )
    elif model_condig['loss'] == 'bpr':
        file_header = '|   layer  |      N-neg    |   train_batch |   recall@20     |    NDCG   |   lr  |  model  |  whether norm| weight_decay |\n'
        file_stats = '|  {} |  {}  |  {}   |   {}    |    {}     |    {}    |    {}   | {} | {} | {}   |   {}   |\n'.format(
            model_condig['n_layers'],  model_condig['num_negtive_items'],model_condig['train_batch'],
            format(res_stat['recall'],'.6f'), format(res_stat['ndcg'],'.6f'),
            format(model_condig['lr'],'.1'),
            model_condig['model'], model_condig['norm_emb'], model_condig['weight_decay'], format(res_stat['test_recall'],'.6f'), format(res_stat['test_ndcg'],'.6f')
        )
    elif model_condig['loss'] == 'softmax':
        if model_condig['mode'] == 'reweight':
            file_header = '|   tmp   |   layer      | neg_weight    |      N-neg    |      recall@20     |    NDCG   |\n'
            file_stats = '|   {}   |    {}   |    {}    |    {}   |   {}    |  {}  |  {}   |  {}   |  {}   |   {}   |  {}   |  {}   | {}   |   {}   |\n'.format(
                format(model_condig['ssm_temp'],'.2f'), model_condig['n_layers'], format(model_condig['neg_coefficient'],'.3f'),  model_condig['num_negtive_items'],
                format(res_stat['recall'],'.6f'), format(res_stat['ndcg'],'.6f'),
                model_condig['search_optimizer'],model_condig['mode'],
                format(model_condig['lr'],'.1'),
                model_condig['model'], model_condig['train_batch'], model_condig['weight_decay'], format(res_stat['test_recall'],'.6f'), format(res_stat['test_ndcg'],'.6f')
            )
        elif model_condig['mode'] == 'multi':
            file_header = '|   tmp2   |   weight   |  layer   |   N-neg    |      recall@20     |    NDCG   | search_optimizer | mode | lr | lr2 | model\n'
            file_stats = '|   {}   |    {}   |    {}    |    {}   |   {}    |  {}  |  {}   | {}   |  {}   |   {}   | {}   |   {}   | {}   |   {}   |\n'.format(
                format(model_condig['ssm_temp2'],'.3f'), format(model_condig['neg_coefficient'],'.3f'), model_condig['n_layers'],  model_condig['num_negtive_items'],
                format(res_stat['recall'],'.6f'), format(res_stat['ndcg'],'.6f'),
                model_condig['search_optimizer'],model_condig['mode'],
                format(model_condig['lr'],'.1'),
                model_condig['model'], model_condig['train_batch'], model_condig['weight_decay'], format(res_stat['test_recall'],'.6f'), format(res_stat['test_ndcg'],'.6f')
