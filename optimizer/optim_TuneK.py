...
class PreAtKOptimizer(IROptimizer):
    def __init__(self, model, config):
        ...
        self.quantile = torch.tensor([config["quantile"]], dtype = torch.float32, device = "cuda")



    def cal_loss(self, users, y_pred: torch.Tensor):
        trunc_pos = y_pred[:,0] - self.quantile
        trunc_neg = y_pred[:,1:] - self.quantile
    ...
    def cal_loss_graph(self,users, pos, neg):
        ...
        loss = self.cal_loss(users, y_pred)
