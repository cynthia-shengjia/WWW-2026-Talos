...
class AATKOptimizer(IROptimizer):
    def __init__(self, model, config):
        ...
        self.activation  = lambda x: torch.relu(x + 1)
    ...
    def cal_loss(self, users, y_pred: torch.Tensor, quantile: torch.Tensor):
        trunc_pos = quantile.detach().squeeze() - y_pred[:,0]
        trunc_neg = y_pred[:,1:] - quantile.detach()

        
        pos_logits = self.activation(trunc_pos)
            
        neg_logits = self.activation(trunc_neg).mean(dim = 1)
