...
class AdvInfoNCEOptimizer(IROptimizer):
    def __init__(self, model, config):
        ...
        # === Model Optimizer ===
        self.opt_model  =   torch.optim.Adam([ {    "params": self.model.parameters(), "lr": self.lr } ], weight_decay = self.weight_decay)
    ...
    def step(self, user, pos, neg, epoch, adv_training_flag):
        ...
        advInfoNCE_loss, additional_loss = self.cal_loss_graph(users = user, pos = pos, neg = neg)
        
        if   adv_training_flag is True:
            loss = additional_loss - advInfoNCE_loss
