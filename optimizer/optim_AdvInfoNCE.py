...
class AdvInfoNCEModel(IROptimizer):
    def __init__(self, model, config):
        ...
        self.eta_epoch          = config["eta_epochs"]
        self.neg_weight         = config["neg_coefficient"]

        self.w_emb_dim          = 64

        # === Model Parameter ===
        self.embed_user_p = (nn.Embedding(self.model.num_users, self.w_emb_dim)).cuda()
        self.embed_item_p = (nn.Embedding(self.model.num_items, self.w_emb_dim)).cuda()
